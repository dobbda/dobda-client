      ...
      if (res.success) {
        ...
        if (queryKey.includes('answers') || queryKey.includes('enquiries')) {
          ...
          queryKey.includes('answers') &&
            setInfCount({ queryKey: keys.questions(), changeKey: 'answersCount', findId: id, upDown: -1 });
          queryKey.includes('enquiries') &&
            setInfCount({ queryKey: keys.outsources(), changeKey: 'enquiriesCount', findId: id, upDown: -1 });
