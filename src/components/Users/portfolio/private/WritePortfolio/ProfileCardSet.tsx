...
import { ImageProp, Maker } from 'src/interface';
...
const ProfileCardSet = ({ data }: Props) => {
  ...
  const [bgColor, setBgColor] = useState<string>(
    data?.card?.bgColor || '#5c27bd',
  );
  const [titleColor, setTitleColor] = useState<string>(
    data?.card?.titleColor || '#ffffff',
  );
  ...
  return (
    <div>
      ...
      <div css={{ fontSize: '13px', color: 'darkcyan' }}>
        이미지나 색상 변경 가능합니다.
      </div>
    </div>
  );
