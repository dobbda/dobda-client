...
export const MyPortfolio = ({}: Props) => {
  ...
  const { data, refetch } = useQuery(keys.maker(auth?.id), () => getPf(auth?.id), {
  ...
  const [position, setPosition] = useState<string[] | string>(data?.position || []);
      ...
      <Wrapper>
        ...
        >
          ...
          <AntSelect
            placeholder="현재 직업"
            defaultValue={job}
            style={{ width: 120, marginBottom: '1.4rem' }}
            options={jobList}
            onChange={(v) => setJob(v)}
          />
          <Label>전문분야</Label>
